...
4. One-way SHA-256 cryptographic hashing via `AuditSensitivityPipeline.HashValue(plainText, salt: tenantSalt)`, which accepts an optional cryptographic salt (SEC-06) to prevent rainbow table attacks against low-entropy data.
5. Support for GDPR Art. 17 (Right to be Forgotten) through the Crypto-Shredding pattern (PRV-02), where PII fields are encrypted under per-actor keys that can be purged to permanently anonymize data without breaking cryptographic HMAC chain continuity.
...
* Cryptographic salt mitigates dictionary and precomputation attacks on sensitive fields.
* Crypto-shredding enables full GDPR compliance without breaking immutable audit trail hashes.
