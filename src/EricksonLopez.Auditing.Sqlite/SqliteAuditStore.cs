...
/// <summary>Provides a SQLite persistence store for immutable audit records using Dapper.</summary>
public sealed class SqliteAuditStore : IAuditStore
{
    private readonly SqliteAuditStoreOptions _options;
    private readonly string _table;

    /// <summary>Initializes a new instance of the <see cref="SqliteAuditStore"/> class.</summary>
    ...
    private (string Sql, DynamicParameters Parameters) BuildQuerySql(AuditQuery query)
    {
        ...
        p.Add("MinDate", (query.From ?? DateTimeOffset.UnixEpoch).ToString("O", CultureInfo.InvariantCulture));

        if (query.To.HasValue) { where.Add("occurred_at <= @MaxDate"); p.Add("MaxDate", query.To.Value.ToString("O", CultureInfo.InvariantCulture)); }
        ...
        {
            where.Add($"""
                (occurred_at > (SELECT occurred_at FROM {_table} WHERE id = @CursorId)
                 OR (occurred_at = (SELECT occurred_at FROM {_table} WHERE id = @CursorId) AND id > @CursorId))
                """);
            p.Add("CursorId", query.AfterRecordId.Value.ToString());
        }
    ...
    private static DynamicParameters ToParameters(AuditRecord record)
    {
        ...
        p.Add("OccurredAt", record.OccurredAt.ToString("O", CultureInfo.InvariantCulture));
