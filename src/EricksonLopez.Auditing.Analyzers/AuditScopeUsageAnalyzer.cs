...
namespace EricksonLopez.Auditing.Analyzers
{
    ...
    public sealed class AuditScopeUsageAnalyzer : DiagnosticAnalyzer
    {
        ...
        private static void AnalyzeNode(SyntaxNodeAnalysisContext context)
        {
            ...
            for (var current = invocationExpr.Parent; current != null; current = current.Parent)
            {
                if (current is UsingStatementSyntax ||
                    (current is LocalDeclarationStatementSyntax localDecl && localDecl.UsingKeyword.IsKind(SyntaxKind.UsingKeyword)))
                {
                    return;
                }
            }
