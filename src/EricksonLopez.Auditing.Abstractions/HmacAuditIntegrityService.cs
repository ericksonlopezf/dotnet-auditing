...
public sealed class HmacAuditIntegrityService
{
    ...
    public string ComputeHash(AuditRecord record, string? previousHash)
    {
        ...
        try
        {
            ...
            // Stryker disable once all : Equivalent mutation - renting buffer when maxByteCount <= 1024 produces identical hash
            ...
            // Stryker disable once all : Equivalent mutation - buffer pool cleanup in finally does not affect computed hash
            finally
            {
        ...
        // Stryker disable once all : Equivalent mutation - charBuffer disposal in finally does not affect computed hash
        finally
        {
            charBuffer.Dispose();
        }
    ...
    private static void BuildCanonicalChars(ref CharBuffer buffer, AuditRecord record, string? previousHash)
    {
        ...
        // Stryker disable once all : Equivalent mutation - when Count is 0, both branches yield '0#'
    ...
    private ref struct CharBuffer
    {
        ...
        {
            // Stryker disable once Equality : Equivalent mutation - growing at exact capacity boundary does not affect buffer contents
            if (_position + length > _buffer.Length)
            {
                // Stryker disable once Arithmetic : Equivalent mutation - geometric growth factor does not affect canonical buffer contents
                ...
                // Stryker disable once all : Equivalent mutation - pool cleanup of old buffer does not affect contents
        ...
        // Stryker disable once all : Equivalent mutation - pool cleanup on dispose does not affect computed hash
        public void Dispose()
        {
            if (_rentedBuffer != null)
            {
