...
public sealed class MySqlAuditStore : IAuditStore
{
    ...
    public MySqlAuditStore(MySqlAuditStoreOptions options)
    {
    ...
    private (string Sql, DynamicParameters Parameters) BuildQuerySql(AuditQuery query)
    {
        ...
        if (query.AfterRecordId.HasValue)
        {
            ...
            where.Add($"""
                (`occurred_at` > (SELECT `occurred_at` FROM `{_options.Table}` WHERE `id` = @CursorId)
                 OR (`occurred_at` = (SELECT `occurred_at` FROM `{_options.Table}` WHERE `id` = @CursorId) AND `id` > @CursorId))
                """);
