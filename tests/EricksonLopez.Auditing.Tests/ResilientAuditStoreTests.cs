...
#pragma warning restore CA2012
    ...
    public async Task AppendBatchAsync_Deferred_NonCritical_SuppressesExceptionAndEnqueuesFallback()
    {
        ...
        var decorator = new ResilientAuditStoreDecorator(faultyStore, config);
        var uniqueAction = $"DEF_BATCH_{Guid.NewGuid():N}";
        var record = AuditRecordBuilder.Create().WithAction(uniqueAction).Build();
        ...
        try
        {
            ...
            var currentFiles = System.IO.Directory.GetFiles(tempDir, "audit-deferred-batch-*.json");
            foreach (var f in currentFiles)
            {
                try
                {
                    if (System.IO.File.ReadAllText(f).Contains(uniqueAction))
                    {
                        matchingFile = f;
                        break;
                    }
            ...
            matchingFile.Should().NotBeNull("a deferred file containing unique record action must be created");
        }
        finally
        {
            if (matchingFile != null)
            {
                try { System.IO.File.Delete(matchingFile); } catch { }
            }
        }
    }

    [Fact]
    public async Task AppendBatchAsync_FailOpen_NonCritical_DoesNotCreateDeferredFile()
    {
        var faultyStore = new FaultyAuditStore();
        var config = new AuditConfiguration { DefaultFailureBehavior = AuditFailureBehavior.FailOpen };
        var decorator = new ResilientAuditStoreDecorator(faultyStore, config);
        var uniqueAction = $"FAILOPEN_BATCH_{Guid.NewGuid():N}";
        var record = AuditRecordBuilder.Create().WithAction(uniqueAction).Build();
        var records = new[] { record };
        ...
        matchingFile.Should().BeNull("FailOpen should not create deferred file");
    }
    ...
    [Fact]
    public async Task AppendBatchAsync_Deferred_WritesDeferredBatchFile()
    {
        var faultyStore = new FaultyAuditStore { ThrowOnAppend = true };
        var config = new AuditConfiguration
        {
        ...
        var uniqueAction = "BATCH_" + Guid.NewGuid().ToString("N");
        var decorator = new ResilientAuditStoreDecorator(faultyStore, config);
        var records = new[] { AuditRecordBuilder.Create().WithAction(uniqueAction).Build() };

        await decorator.AppendBatchAsync(records);

        var tempFiles = System.IO.Directory.GetFiles(System.IO.Path.GetTempPath(), "audit-deferred-batch-*.json");
    ...
    [Fact]
    public async Task AppendBatchAsync_FailOpen_DoesNotWriteDeferredBatchFile()
    {
        var faultyStore = new FaultyAuditStore { ThrowOnAppend = true };
        var config = new AuditConfiguration
        {
        ...
        var uniqueAction = "FAILOPEN_" + Guid.NewGuid().ToString("N");
        var decorator = new ResilientAuditStoreDecorator(faultyStore, config);
        var records = new[] { AuditRecordBuilder.Create().WithAction(uniqueAction).Build() };
