...
public sealed class DiRegistrationTests
{
    ...
    [Fact]
    public void AdapterDi_UsePostgreSql_RegistersStoreAndVerifier()
    {
        var services = new ServiceCollection();
        ...
        builder.UsePostgreSql(opt =>
        {
            opt.ConnectionFactory = () => new FakeDbConnection();
        });

        var sp = services.BuildServiceProvider();
        sp.GetService<IAuditStore>().Should().BeOfType<PostgreSqlAuditStore>();
        sp.GetService<PostgreSqlAuditIntegrityVerifier>().Should().NotBeNull();
    }

    [Fact]
    public void AdapterDi_UseSqlServer_RegistersStoreAndVerifier()
    {
        var services = new ServiceCollection();
        services.AddSingleton<IAuditIntegrityProvider, TestAuditIntegrityProvider>();
        services.AddSingleton<HmacAuditIntegrityService>();
        ...
        builder.UseSqlServer(opt =>
        {
            opt.ConnectionFactory = () => new FakeDbConnection();
        });

        var sp = services.BuildServiceProvider();
        sp.GetService<IAuditStore>().Should().BeOfType<SqlServerAuditStore>();
        sp.GetService<SqlServerAuditIntegrityVerifier>().Should().NotBeNull();
    }

    [Fact]
    public void AdapterDi_UseSqlite_RegistersStoreAndVerifier()
    {
        var services = new ServiceCollection();
        services.AddSingleton<IAuditIntegrityProvider, TestAuditIntegrityProvider>();
        services.AddSingleton<HmacAuditIntegrityService>();
        ...
        builder.UseSqlite(opt =>
        {
            opt.ConnectionFactory = () => new FakeDbConnection();
        });

        var sp = services.BuildServiceProvider();
        sp.GetService<IAuditStore>().Should().BeOfType<SqliteAuditStore>();
        sp.GetService<SqliteAuditIntegrityVerifier>().Should().NotBeNull();
    }

    [Fact]
    public void AdapterDi_UseMySql_RegistersStoreAndVerifier()
    {
        var services = new ServiceCollection();
        services.AddSingleton<IAuditIntegrityProvider, TestAuditIntegrityProvider>();
        services.AddSingleton<HmacAuditIntegrityService>();
        ...
        var sp = services.BuildServiceProvider();
        sp.GetService<IAuditStore>().Should().BeOfType<MySqlAuditStore>();
        sp.GetService<MySqlAuditIntegrityVerifier>().Should().NotBeNull();
    }

    [Fact]
    public void AdapterDi_UseOracle_RegistersStoreAndVerifier()
    {
        var services = new ServiceCollection();
        services.AddSingleton<IAuditIntegrityProvider, TestAuditIntegrityProvider>();
        services.AddSingleton<HmacAuditIntegrityService>();
        ...
        builder.UseOracle(opt =>
        {
            opt.ConnectionFactory = () => new FakeDbConnection();
        });

        var sp = services.BuildServiceProvider();
        sp.GetService<IAuditStore>().Should().BeOfType<OracleAuditStore>();
        sp.GetService<OracleAuditIntegrityVerifier>().Should().NotBeNull();
    }
