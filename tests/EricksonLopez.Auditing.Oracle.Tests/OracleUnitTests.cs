...
public sealed class OracleUnitTests
{
    ...
    [Fact]
    public void OracleAuditStoreOptions_SchemaAndTable_Validation()
    {
        var options = new OracleAuditStoreOptions();
        options.Schema.Should().Be("AUDIT_USER");
        options.Table.Should().Be("AUDIT_RECORDS");
    ...
    public async Task AppendBatchAsync_EmptyBatch_ReturnsWithoutExecuting()
    {
        ...
        var store = new OracleAuditStore(new OracleAuditStoreOptions { ConnectionFactory = () => conn });
        await store.AppendBatchAsync(Array.Empty<AuditRecord>());
    ...
    public async Task Append_And_AppendBatch_WithSensitivityPipeline_SanitizesRecords()
    {
        ...
        var store = new OracleAuditStore(new OracleAuditStoreOptions { ConnectionFactory = () => conn }, pipeline);
    ...
    public async Task AppendBatchAsync_WithNonDbConnection_OpensAndBeginsTransaction()
    {
        ...
        var store = new OracleAuditStore(new OracleAuditStoreOptions { ConnectionFactory = () => wrapper });
    ...
    public async Task Verifier_WithNonDbConnection_OpensConnection()
    {
        ...
        innerConn.Close();
        var wrapper = new NonDbConnectionWrapper(innerConn);
        var verifier = new OracleAuditIntegrityVerifier(new OracleAuditStoreOptions { ConnectionFactory = () => wrapper }, _hmac);

        var r = AuditRecordBuilder.BuildDefault(tenantId: "tenant-nondb");
        var hash = _hmac.ComputeHash(r, null);
        r = r with { IntegrityHash = hash };

        innerConn.ReaderQueues.Enqueue(_ => FakeDbDataReaderFactory.CreateOracle(r));
    ...
    public async Task Verifier_RecordsWithChanges_IncludingRedacted_VerifyCorrectly()
    {
        ...
        conn.ReaderQueues.Enqueue(_ => FakeDbDataReaderFactory.CreateOracle(signed));
        var verifier = new OracleAuditIntegrityVerifier(new OracleAuditStoreOptions { ConnectionFactory = () => conn }, _hmac);
    ...
    public async Task QueryAsync_WithContinuationToken_SetsCursorDateAndCursorIdParameters()
    {
        ...
        var store = new OracleAuditStore(new OracleAuditStoreOptions { ConnectionFactory = () => conn });
        var cursorId = Guid.NewGuid();
        var cursorDate = new DateTimeOffset(2026, 1, 15, 12, 30, 45, 123, TimeSpan.Zero);

        conn.ReaderQueues.Enqueue(_ => FakeDbDataReaderFactory.CreateOracle(Array.Empty<AuditRecord>()));
        ...
        conn.ExecutedCommands.Should().Contain(c => c.CommandText.Contains("TO_TIMESTAMP(:CursorDate, 'YYYY-MM-DD HH24:MI:SS.FF6')"));
        var queryCmd = conn.ExecutedCommands.First(c => c.CommandText.Contains("TO_TIMESTAMP(:CursorDate"));
        queryCmd.Parameters["CursorDate"].Value.Should().Be(cursorDate.UtcDateTime.ToString("yyyy-MM-dd HH:mm:ss.ffffff", System.Globalization.CultureInfo.InvariantCulture));
        queryCmd.Parameters["CursorId"].Value.Should().Be(cursorId.ToString("N").ToUpperInvariant());
    }
    ...
    public async Task AppendAsync_WithIdempotencyKey_SetsIdempotencyKeyParameter()
    {
        ...
        var r = AuditRecordBuilder.Create()
            ...
            .Build();
        r = r with { Context = r.Context with { IdempotencyKey = "idem-key-oracle" } };

        await store.AppendAsync(r);

        var insertCmd = fakeConn.ExecutedCommands.First(c => c.CommandText.Contains("INSERT INTO"));
        insertCmd.Parameters["IdempotencyKey"].Value.Should().Be("idem-key-oracle");
    }

    [Fact]
    public async Task Store_EnforcesAsyncTransaction_OnAppendAndQuery()
    {
        var conn = new FakeDbConnection { EnforceAsyncTransaction = true };
        var store = new OracleAuditStore(new OracleAuditStoreOptions { ConnectionFactory = () => conn });
        var record = AuditRecordBuilder.BuildDefault("tenant-async-tx");
        ...
        var queryResult = await store.QueryAsync(new AuditQuery { TenantId = "tenant-async-tx" });
    ...
    public async Task Verifier_RecordsWithNullAndEmptyChangesJson_VerifyCorrectly()
    {
        ...
        conn.ReaderQueues.Enqueue(_ => FakeDbDataReaderFactory.CreateWithRawChanges(signed, "null", isStringId: true, isStringDate: false, isOracle: true));
        var verifier = new OracleAuditIntegrityVerifier(new OracleAuditStoreOptions { ConnectionFactory = () => conn }, _hmac);
        ...
        conn.ReaderQueues.Enqueue(_ => FakeDbDataReaderFactory.CreateWithRawChanges(signed, "[]", isStringId: true, isStringDate: false, isOracle: true));
