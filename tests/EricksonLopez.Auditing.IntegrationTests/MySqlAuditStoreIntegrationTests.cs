...
public sealed class MySqlAuditStoreIntegrationTests : IAsyncLifetime
{
    private readonly MySqlContainer _container;
    private MySqlAuditStore _store = null!;
    private MySqlAuditIntegrityVerifier _verifier = null!;
    private HmacAuditIntegrityService _hmac = null!;

    public MySqlAuditStoreIntegrationTests()
    {
    ...
    public async Task InitializeAsync()
    {
        ...
        using (var conn = new MySqlConnection(_container.GetConnectionString() + ";Allow User Variables=true"))
        {
        ...
        {
            ConnectionFactory = () => new MySqlConnection(_container.GetConnectionString() + ";Allow User Variables=true"),
            Table = "audit_records"
        };

        var provider = services.BuildServiceProvider();
        _hmac = provider.GetRequiredService<HmacAuditIntegrityService>();
        _store = new MySqlAuditStore(options);
        _verifier = new MySqlAuditIntegrityVerifier(options, _hmac);
    }
    ...
    public async Task VerifyChain_BrokenChain_ReturnsFalse()
    {
        ...
        {
            await conn.OpenAsync();
            await conn.ExecuteAsync("UPDATE audit_records SET integrity_hash = 'tampered' WHERE id = @Id", new { Id = r1.Id });
        }
