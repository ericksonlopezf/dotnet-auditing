...
namespace EricksonLopez.Auditing.MySql.Tests;
...
public sealed class MySqlUnitTests
{
    ...
    public void Extensions_NullArguments_Throw()
    {
        ...
        Assert.Throws<ArgumentNullException>(() => MySqlAuditExtensions.UseMySql(null!, opt => { }));
    ...
    public void Extensions_UnconfiguredConnectionFactory_ThrowsOnInvocation()
    {
        var services = new ServiceCollection();
        var builder = services.AddAuditing();
        builder.UseMySql(opt => { /* leave ConnectionFactory unconfigured */ });

        var sp = services.BuildServiceProvider();
        var options = sp.GetRequiredService<MySqlAuditStoreOptions>();
        Action act = () => options.ConnectionFactory();
        var ex = Assert.Throws<InvalidOperationException>(act);
        ex.Message.Should().Be("MySqlAuditStoreOptions.ConnectionFactory must be configured. Call UseMySql(options => options.ConnectionFactory = () => new MySqlConnection(...)).");
    }
    ...
    public void Extensions_RegistersServicesCorrectly()
    {
        ...
        store.Should().BeOfType<MySqlAuditStore>();

        var verifier = sp.GetService<MySqlAuditIntegrityVerifier>();
    ...
    {
        Assert.Throws<ArgumentNullException>(() => new MySqlAuditStore(null!));
    }
    ...
    public async Task Store_AppendAsync_NullRecord_Throws()
    {
        ...
        var store = new MySqlAuditStore(new MySqlAuditStoreOptions { ConnectionFactory = () => fakeConn });
    ...
    {
        var fakeConn = new FakeDbConnection();
        var store = new MySqlAuditStore(new MySqlAuditStoreOptions
        {
        ...
        rlsCmd.Parameters["TenantId"].Value.Should().Be("tenant-mysql-1");

        var insertCmd = fakeConn.ExecutedCommands[1];
        insertCmd.CommandText.Should().Contain("INSERT INTO `my_records`");
        insertCmd.Parameters["Id"].Value.Should().Be(id.ToString("D"));
        insertCmd.Parameters["OccurredAt"].Value.Should().Be(now.UtcDateTime);
        insertCmd.Parameters["TenantId"].Value.Should().Be("tenant-mysql-1");
    ...
    public async Task Store_QueryAsync_AllFilters_GeneratesExpectedSqlAndParameters()
    {
        ...
        queryCmd.CommandText.Should().Contain("FROM `events`");
    ...
    public async Task Store_QueryAsync_RowMappingAndPagination()
    {
        ...
        fakeConn.ReaderQueues.Enqueue(_ => FakeDbDataReaderFactory.Create(new[] { r1, r2, r3 }, isStringId: true));
    ...
    {
        var options = new MySqlAuditStoreOptions { ConnectionFactory = () => new FakeDbConnection() };
        Assert.Throws<ArgumentNullException>(() => new MySqlAuditIntegrityVerifier(null!, _hmac));
        Assert.Throws<ArgumentNullException>(() => new MySqlAuditIntegrityVerifier(options, null!));
    }
    ...
    {
        var fakeConn = new FakeDbConnection();
        var verifier = new MySqlAuditIntegrityVerifier(new MySqlAuditStoreOptions { ConnectionFactory = () => fakeConn }, _hmac);
    ...
    public async Task Verifier_VerifyChainAsync_ValidChain_Succeeds()
    {
        ...
        var tenant = "tenant-v";
        var r1 = AuditRecordBuilder.Create().WithTenant(tenant).WithResource("R", "1").Build();
        var hash1 = _hmac.ComputeHash(r1, null);
        r1 = r1 with { IntegrityHash = hash1, PreviousHash = null };

        var r2 = AuditRecordBuilder.Create().WithTenant(tenant).WithResource("R", "2").Build();
        var hash2 = _hmac.ComputeHash(r2, hash1);
        r2 = r2 with { IntegrityHash = hash2, PreviousHash = hash1 };

        fakeConn.ReaderQueues.Enqueue(_ => FakeDbDataReaderFactory.Create(new[] { r1, r2 }, isStringId: true));
        ...
        fakeConn.ExecutedCommands.Should().HaveCount(1);
        var queryCmd = fakeConn.ExecutedCommands[0];
        queryCmd.CommandText.Should().Contain("FROM `audit_records`");
    ...
    public async Task Verifier_VerifyChainAsync_TamperedHash_ReturnsFalse()
    {
        ...
        var r1 = AuditRecordBuilder.Create().WithTenant(tenant).WithResource("R", "1").Build() with
        {
            IntegrityHash = "INVALID_HASH",
            PreviousHash = null
        };

        fakeConn.ReaderQueues.Enqueue(_ => FakeDbDataReaderFactory.Create(new[] { r1 }, isStringId: true));
    ...
    public async Task Verifier_VerifyChainAsync_Cancellation_Throws()
    {
        ...
        var rCancel = AuditRecordBuilder.Create().WithTenant("tenant").Build();
        fakeConn.ReaderQueues.Enqueue(_ => FakeDbDataReaderFactory.Create(new[] { rCancel }, isStringId: true));
    ...
    public async Task Verifier_VerifyChainAsync_SubchainWithPreviousHash_Succeeds()
    {
        ...
        fakeConn.ReaderQueues.Enqueue(_ => FakeDbDataReaderFactory.Create(new[] { rSub }, isStringId: true));
    ...
    public async Task Store_And_Verifier_WithClosedAndOpenConnections_BehaveCorrectly()
    {
        ...
        closedConn.Close();
        var store = new MySqlAuditStore(new MySqlAuditStoreOptions { ConnectionFactory = () => closedConn });
        var verifier = new MySqlAuditIntegrityVerifier(new MySqlAuditStoreOptions { ConnectionFactory = () => closedConn }, _hmac);
        ...
        closedConn.Close();
        closedConn.ReaderQueues.Enqueue(_ => FakeDbDataReaderFactory.Create(new[] { r }, isStringId: true));
        ...
        openConn.Open();
        var storeOpen = new MySqlAuditStore(new MySqlAuditStoreOptions { ConnectionFactory = () => openConn });
        var verifierOpen = new MySqlAuditIntegrityVerifier(new MySqlAuditStoreOptions { ConnectionFactory = () => openConn }, _hmac);
    ...
    public async Task QueryAsync_WithContinuationToken_SetsCursorDateAndCursorIdParameters()
    {
        ...
        var queryCmd = conn.ExecutedCommands.First(c => c.CommandText.Contains("occurred_at > @CursorDate"));
        queryCmd.Parameters["CursorDate"].Value.Should().Be(cursorDate.ToString("yyyy-MM-dd HH:mm:ss.ffffff", CultureInfo.InvariantCulture));
    ...
    public async Task AppendAsync_WithIdempotencyKey_PopulatesParameter()
    {
        ...
        var insertCmd = conn.ExecutedCommands.First(c => c.CommandText.Contains("INSERT INTO"));
        insertCmd.Parameters["IdempotencyKey"].Value.Should().Be("unique-key-123");
        conn.CreatedTransactions.Should().OnlyContain(t => t.Committed);
    }
    ...
    public async Task Verifier_VerifyChainAsync_CancellationDuringIteration_Throws()
    {
        ...
        {
            cts.Cancel();
            return FakeDbDataReaderFactory.Create(new[] { signed }, isStringId: true);
        });
    ...
    public async Task Verifier_RecordsWithNullAndEmptyChangesJson_VerifyCorrectly()
    {
        ...
        conn.ReaderQueues.Enqueue(_ => FakeDbDataReaderFactory.CreateWithRawChanges(signed, "null", isStringId: true, isStringDate: false));
        var verifier = new MySqlAuditIntegrityVerifier(new MySqlAuditStoreOptions { ConnectionFactory = () => conn }, _hmac);
        ...
        conn.ReaderQueues.Enqueue(_ => FakeDbDataReaderFactory.CreateWithRawChanges(signed, "[]", isStringId: true, isStringDate: false));
