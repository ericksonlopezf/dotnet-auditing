...
<#
.SYNOPSIS
    Automated zero-tolerance repository compliance & governance verifier for EricksonLopez.Auditing.
.DESCRIPTION
    Validates:
    ...
    6. Directory.Build.props standards (ImplicitUsings=disable, PackageProjectUrl, icon.png).
...
foreach ($cs in $srcCsFiles) {
    ...
    foreach ($line in $lines) {
        if ($line -match "^(public|internal|protected)\s+(sealed\s+|abstract\s+|static\s+|readonly\s+)*(class|interface|struct|record|enum|delegate)\s+([A-Za-z0-9_]+)") {
            $typeCount++
        }
    }
    if ($typeCount -gt 1) {
...
if ($propsContent -notmatch "<ImplicitUsings>disable</ImplicitUsings>") {
    Write-Host "  ❌ Directory.Build.props must specify <ImplicitUsings>disable</ImplicitUsings>." -ForegroundColor Red
    $violations++
}
if ($propsContent -notmatch "<PackageProjectUrl>https://ericksonlopez\.dev/auditing</PackageProjectUrl>") {
    Write-Host "  ❌ PackageProjectUrl must be set to https://ericksonlopez.dev/auditing." -ForegroundColor Red
    $violations++
}
